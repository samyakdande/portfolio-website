...
export function MagneticText({ 
  ...
  hoverText = "EXPLORE", 
  className,
  baseTextClassName = "text-5xl font-bold tracking-tighter text-foreground tracking-wide",
  hoverTextClassName = "text-5xl font-bold tracking-tighter text-background whitespace-nowrap tracking-wide",
  cursorSize = 150
}: MagneticTextProps) {
  ...
  return (
    ...
    >
      ...
      <span className={baseTextClassName}>{text}</span>

      <div
        ref={circleRef}
        className="absolute top-0 left-0 pointer-events-none rounded-full bg-foreground overflow-hidden"
        style={{
          width: isHovered ? cursorSize : 0,
          height: isHovered ? cursorSize : 0,
      ...
      >
        ...
        >
          <span className={hoverTextClassName}>
            {hoverText}
          </span>
        </div>
      </div>
